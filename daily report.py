...
conn = sqlite3.connect(r'C:\Users\hhrh1\Desktop\Top_K_Hottest\TopK.db')
...
for BoradURL in BoardConfigs:
    ...
    # start web browser
    driver = webdriver.Edge("C:\\Users\\hhrh1\\Desktop\\edgedriver_win64\\msedgedriver.exe")
    # get source code
    driver.get(BoradURL)
    ...
    tbody = soup.find("html").find("body").find("section", id="main").find("section", id="body").find("div", class_="b-content").find("table", class_="board-list tiz").find("tbody")
    ...
    for tr in tr_list:
        ...
        row = cursor.fetchone()
        if row is None:
            cursor.execute("INSERT INTO DailyReport (URL, cnt) VALUES (?,?)", ("\'"+relative_path+"\'", latest_cnt))
