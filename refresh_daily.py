...
def report_daily():
    ...
    for MailAddress in config:
        ...
        for BoardURL in config[MailAddress]:
            TopK = config[MailAddress][BoardURL]["TopK"]
            list_diff = []
            ...
            # sort the list in descending order according to the first element, diff
            list_diff.sort(key=lambda x: x[2], reverse=True)

            current_ret[BoardURL] = list_diff[0:TopK]
        ...
        msg['From'] = my_sender
        msg['To'] = MailAddress
        msg['Subject'] = 'daily report ' + str(datetime.now())
    ...
    with open("/home/laphy/Top_K_Hottest/daily_result.html", "w", encoding='utf-8') as o:
        o.write(content)
